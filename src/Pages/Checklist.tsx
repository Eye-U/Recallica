...
import { Plus, Trash2, Pencil, Check, X, ChevronLeft } from "lucide-react"; // Added ChevronLeft
...
function Checklist() {
  ...
  // NEW STATE: Controls whether we show the calendar or the task list
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, "users", uid, "toDo"), newTask);
      setTask("");
      setAdding(false);
  ...
  const renderDayWithDot = (day: number, date: Date) => {
    const formattedDate = format(date, "MMMM d, yyyy");
    const hasTask = taskList.some((t) => t.date === formattedDate);
    ...
    <div className="checklist-page-container">
      ...
      <main className="main-content">
        <div className="cl-layout">
          ...
          {/* TOGGLE LOGIC: Show Calendar OR Tasks */}
          {!showTasks ? (
                ...
                <DatePicker
                  ...
                  onChange={(date: Date | null) => {
                    if (date) {
                      ...
                      setShowTasks(true); // Switch to tasks view
                    }
                  }}
                  dateFormat="MMMM d, yyyy"
                  inline
                  calendarClassName="cl-datepicker"
                  renderDayContents={renderDayWithDot}
                />
              </div>
            </aside>
          ) : (
            <section className="cl-tasks-section">
              {/* Back Button */}
              <div className="cl-tasks-top-bar">
                <button className="cl-back-btn" onClick={() => setShowTasks(false)}>
